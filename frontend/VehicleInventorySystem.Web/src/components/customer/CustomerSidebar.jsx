...
const CustomerSidebar = ({ user }) => {
  ...
  const initials = (user?.name || 'CU').split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase();
  ...
  return (
    <aside className="staff-sidebar">
      ...
      <div style={{ flex: 1, paddingBottom: '12px' }}>
        ...
        <NavItem to="/customer" icon={LayoutDashboard} label="Dashboard" />
        
        <div className="nav-section-label">My Garage</div>
        <NavItem to="/customer/vehicles" icon={Car} label="My Vehicles" />
        <NavItem to="/customer/history" icon={History} label="Service History" />
      ...
      <div style={{ borderTop: '1px solid #E2E8F0', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: '10px', background: '#FAFBFD' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: '11px', color: '#94A3B8', marginTop: '1px' }}>Customer</div>
        </div>
      </div>
