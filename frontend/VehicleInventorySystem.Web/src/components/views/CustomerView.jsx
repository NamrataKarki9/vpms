...
export function CustomerView({ user }) {
  ...
  const loadData = async () => {
    ...
    const apps = await apiFetch(`/Service/appointments?customerId=${user.id}`);
    if (apps) setAppointments(apps);

    const reqs = await apiFetch(`/Service/part-requests?customerId=${user.id}`);
    if (reqs) setPartRequests(reqs);
  };
  ...
  const handleBookAppointment = async (form) => {
    const { apiFetch } = await import('../../api');
    const newApp = await apiFetch('/Service/appointments', {
      method: 'POST',
      body: JSON.stringify({
        customerId: user.id,
        vehicleId: 1, // To do: select actual vehicle
    ...
    setSubView('appointments');
  };

  const handleRequestPart = async (form) => {
    const { apiFetch } = await import('../../api');
    const newReq = await apiFetch('/Service/part-requests', {
      method: 'POST',
      body: JSON.stringify({
        customerId: user.id,
    ...
    setPartRequests([...partRequests, newReq]);
    setSubView('requests');
  };
  ...
  if (subView === 'book') return <BookingPage user={user} onComplete={handleBookAppointment} onBack={() => setSubView('main')} />;
  ...
  if (subView === 'new-request') return <NewRequestPage user={user} onComplete={handleRequestPart} onBack={() => setSubView('main')} />;
    ...
    <>
      ...
      <div style={{ position: 'fixed', bottom: '22rem', right: '3rem', zIndex: 10000 }}>
        {!showChat ? (
          ...
          <div style={{ width: '420px', height: '580px', background: '#fff', borderRadius: '20px', boxShadow: '0 12px 48px rgba(0,0,0,0.15)', display: 'flex', flexDirection: 'column', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
